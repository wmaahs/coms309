...
void io_teleport_world(pair_t dest)
{
  ...
  do {
    ...
    refresh();
    mvscanw(0, 21,(char *) "%d", &dest_map[dim_x]);
  } while (dest_map[dim_x] < -200 || dest_map[dim_x] > 200);
  do {
    mvprintw(0, 0, "Enter y [-200, 200]:          ");
    refresh();
    mvscanw(0, 21,(char *) "%d", &dest_map[dim_y]);
  } while (dest_map[dim_y] < -200 || dest_map[dim_y] > 200);
